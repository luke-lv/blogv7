...
CutString.prototype.searchEnd = function(parseDiv, newParent){
    ...
    for(var j=0; j< parseDiv.childNodes.length; j++){
        ...
        if(Core.String.byteLength(ele.nodeValue) + this.charCount >= this.limit){
            newEle = ele.cloneNode(true);
        ...
        newEle = ele.cloneNode(true);
        newParent.appendChild(newEle);
        this.charCount += Core.String.byteLength(ele.nodeValue);
    }
    return false;
};

Core.String.cutHtmlString = function($string,$limit){
