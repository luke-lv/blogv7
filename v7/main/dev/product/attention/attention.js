...
scope.Attention.prototype = {
     ...
     */
    add: function (uid, aid, fCallback) {
        var _this = this;
            ...
            onComplete: function (data) {
                ...
                if (data.code == "A33004") {
                    if (fCallback) {
                        ...
                        if (Core.String.trim($E("suggestBody").innerHTML).replace(/"/g, "").toLowerCase() == '<div class=clearit></div>') {
                            $E("suggestBody").parentNode.innerHTML = '<div class="SG_nodata">暂无推荐!</div>';
                        }
                    } else {
                        winDialog.alert($ATTENTION_MSG[data.code], {
                            ...
                            subText: ['<div class="CP_w_cnt SG_txtb">你将在关注动态页面接收到对方的新消息</div>'
                                ...
                                ,
                                    '<li><a href="http://profile.blog.sina.com.cn/attention_feed.php?uid=' + $UID + '" onclick="winDialog.getDialog(\'attention\').close();">查看关注动态&gt;&gt;</a></li>'
                                ,
                                    '<li><a href="http://profile.blog.sina.com.cn/attention.php?uid=' + $UID + '&type=0" onclick="winDialog.getDialog(\'attention\').close();">查看我关注的人&gt;&gt;</a></li>'
                                ,
                                    '<li><a href="http://profile.blog.sina.com.cn/attention.php?uid=' + $UID + '&type=1" onclick="winDialog.getDialog(\'attention\').close();">查看谁在关注我&gt;&gt;</a></li>'
                                , '</ul>'].join(""),
                            funcOk: function () {
