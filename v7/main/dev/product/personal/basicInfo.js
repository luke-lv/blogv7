...
scope.BasicInfo = {
    ...
    initialize: function () {
        ...
        //初始化错误消息提示管理器
        var failedMsgMgr = new scope.personal.FailedMsgManager(["msgBirthday", "msgSameBirthday", "msgPlace",
            "msgSamePlace", "msgHometown", "msgSameHometown"]);
        ...
        function initializeBasicInfo(isHistory) {
            ...
            //更新"同生缘"的操作
            function updateSameBirthday() {
                Core.Events.addEvent($E("sameBirthday"), showSameBirthdayFailedMsg, "click");
                if (verifyBirthday()) {
                    var y = selBirthdayYear.options[selBirthdayYear.selectedIndex].innerHTML;
                    var m = selBirthdayMonth.options[selBirthdayMonth.selectedIndex].innerHTML;
                    var d = selBirthdayDay.options[selBirthdayDay.selectedIndex].innerHTML;
                    $E("sameBirthday").href = "http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&byea=" + y + "&bmon=" + _this.formatNumber(m) + _this.formatNumber(d);
                    $E("sameBirthday").target = "_blank";
                } else {
                    $E("sameBirthday").target = "_self";
                    $E("sameBirthday").href = "javascript:;";
                }
            }
            ...
            //更新"找邻居"的操作
            function updateSamePlace() {
                Core.Events.addEvent($E("samePlace"), showSamePlaceFailedMsg, "click");
                if (verifyProvinceAndCity(selProvince, selCity) && selProvince.value != "0") {
                    var p1 = selProvince.value;
                    var p2 = selProvince.value != "1001" ? selCity.value : "";
                    $E("samePlace").href = "http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&prov=" + p1 + "&city=" + p2;
                    $E("samePlace").target = "_blank";

                } else {
                    $E("samePlace").target = "_self";
                    $E("samePlace").href = "javascript:;";
                }
            }

            function showSamePlaceFailedMsg() {
                if (!verifyProvinceAndCity(selProvince, selCity) || selProvince.value == "0") {
                    failedMsgMgr.hiddenFailed("msgPlace");
            ...
            //更新"找老乡"的操作
            function updateSameHometown() {
                Core.Events.addEvent($E("sameHometown"), showSameHometownFailedMsg, "click");
                if (verifyProvinceAndCity(selHometownProvince, selHometownCity) && selHometownProvince.value != "0") {
                    var p1 = selHometownProvince.value;
                    var p2 = selHometownProvince.value != "1001" ? selHometownCity.value : "";
                    $E("sameHometown").href = "http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&bpro=" + p1 + "&bcit=" + p2;
                    $E("sameHometown").target = "_blank";
                } else {
                    $E("sameHometown").href = "javascript:;";
                    $E("sameHometown").target = "_self";
                }
            }
        ...
        function hiddenBasicInfoEditor() {
            ...
            if (!haveData) {
                ...
                if ($E("basicAddItem")) {
                    ...
                    $E("basicInfoRes").innerHTML = '<table class="personTable">' + '<tbody>' + '<tr id="basicAddItem">' + '<td><p class="personIntro CP_txta">' + '暂无基本信息，<strong><a href="javascript:;"><cite id="linkAddBasicInfo">请添加</cite></a>。</strong></p></td>' + '</tr>' + '</tbody>' + '</table>';
                    Core.Events.addEvent($E("linkAddBasicInfo"), showBasicInfoEditor, "click");
                }
            }
        }
        ...
        function verifyBirthday() {
            return $E("selBirthdayYear").selectedIndex != 0 && $E("selBirthdayMonth").selectedIndex != 0 && $E("selBirthdayDay").selectedIndex != 0;
        }
...
//			(new Interface("http://icp.cws.api.sina.com.cn/profile/show_baseDetail.php", "jsload")).request({
            ...
            function _updateShowData(iData) {
                ...
                if (iData["sex"]) {
                    haveData = true;
                    valueRows.push('<tr>' + '<td class="rolTd1 CP_txta">性   别：</td>' + '<td class="rolTd2"><span class="tdP20"><a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&gend=' + searchSex + '">' + iData["sex"] + '</a></span></td>' + '</tr>');
                }
                ...
                if (iData["fullDate"] != "2" && iData["year"] && iData["month"] && iData["day"]) {
                    haveData = true;
                    var hashDate = {
                        "0": '<a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&byea=' + iData["year"] + '">' + iData["year"] + '年</a> ' + '<a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&bmon=' + _this.formatNumber(iData["month"]) + _this.formatNumber(iData["day"]) + '">' + iData["month"] + '月' + iData["day"] + '日</a> ' + '<a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&name=' + hashSearchAstro[unescape(iData["astro"])] + '">' + iData["astro"] + '</a>',
                        "1": '<a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&bmon=' + _this.formatNumber(iData["month"]) + _this.formatNumber(iData["day"]) + '">' + iData["month"] + '月' + iData["day"] + '日</a>'
                    };
                    valueRows.push('<tr>' + '<td class="rolTd1 CP_txta">生   日：</td>' + '<td class="rolTd2"><span class="tdP20">' + hashDate[iData["fullDate"]] + '</span></td>' + '</tr>');
                }

                //婚姻
                if (iData["marriage"]) {
                    haveData = true;
                    valueRows.push('<tr>' + '<td class="rolTd1 CP_txta">婚   姻：</td>' + '<td class="rolTd2"><span class="tdP20"><a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&im=' + searchMarriage + '">' + iData["marriage"] + '</a></span></td>' + '</tr>');
                }

                //职业
                if (iData["job"]) {
                    haveData = true;
                    valueRows.push('<tr>' + '<td class="rolTd1 CP_txta">职   业：</td>' + '<td class="rolTd2"><span class="tdP20"><a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&r3=' + searchJob + '">' + iData["job"] + '</a></span></td>' + '</tr>');
                }

                //现居地
                if (iData["place1"]) {
                    haveData = true;
                    valueRows.push('<tr>' + '<td class="rolTd1 CP_txta">现居地：</td>' + '<td class="rolTd2"><span class="tdP20"><a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&prov=' + searchPlace1 + '">' + iData["place1"] + '</a></span>');
                    if (iData["place2"]) {
                        valueRows.push('<span class="tdP20"><a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&prov=' + searchPlace1 + '&city=' + searchPlace2 + '">' + iData["place2"] + '</a></span>');
                    }
                ...
                if (iData["hometown1"]) {
                    haveData = true;
                    valueRows.push('<tr>' + '<td class="rolTd1 CP_txta">家   乡：</td>' + '<td class="rolTd2"><span class="tdP20"><a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&bpro=' + searchHometown1 + '">' + iData["hometown1"] + '</a></span>');
                    if (iData["hometown2"]) {
                        valueRows.push('<span class="tdP20"><a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&bpro=' + searchHometown1 + '&bcit=' + searchHometown2 + '">' + iData["hometown2"] + '</a></span>');
                    }
                    valueRows.push('</td></tr>');
                }
