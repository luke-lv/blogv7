...
$registJob('digger',function(){
	...
	var diggerCite = $E("d1_digg_" + scope.$articleid);
	...
	scope.digger.showLast10 = function(opt) {
		...
		opt.onSuccess = function(data) {
			...
			for(var i=0; data[i]; i++) {
				...
				pers.push('<div>',
                    '<a target="_blank" title="到' ,per['action_uname'] ,'的博客去看看" href="http://blog.sina.com.cn/u/', per['action_uid'], '">' ,sName ,'</a>'
                    ,getVHTML(parseInt(per.wtype || -1))
                    ,'</div>');
			}
			if(!pers.length) {
				if(diggerCite && diggerCite.innerHTML === "顶"){
