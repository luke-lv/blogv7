 ...
 *
 */
$registJob("errorTips_new", function(){
    Lib.checkAuthor();
            ...
            onComplete:function(result){
                ...
                if(result && result.iswarning == 1){
                    ...
                    scope.i8t9c = function(){
                        ...
                        Utils.Cookie.setCookie('i8t9c'+$UID, result.date, 8760, '/'); //为游戏频道设置cookie,时间为一年
                        Utils.Cookie.setCookie('i8t9c'+$UID, result.date, 8760, '/','blog.sina.com.cn'); //为与个人中心页共享cookie
                    };
                    scope.i8t9c.show = function(){
                        $E('err_i8t9c').style.display = '';
                        updatePos();
                    };
                    errorTips = createNode(html);
                    document.body.appendChild(errorTips);
                    
                    var act = $E('loginBarActivity'), txt;
                    target = createNode('<div class="topbar_activity"><a href="javascript:;" onclick="scope.i8t9c&&scope.i8t9c.show();">[公告]</a></div>');
                ...
                function updatePos(){
                    ...
                    errorTips.style.top=pos[1]+40+"px";
