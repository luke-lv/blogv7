		...
		login: function(){
			...
			_login = new Lib.Login.LoginPost(function(res){
				...
				if(res.result){
					...
					Core.Events.fireEvent($E('movingBtn_ihave'),'click');
				}else{
...
//			this.inProgress = true;
	...
	function toMsnLive(res){
		...
		function __toMsnLive(){
			/*
			var uid = __getCurrectUID();
			if (uid){
				window.location.href = "http://blog.sina.com.cn/u/" + uid;				// 跳到绑定的博客首页去
			}else{
				window.location.href = "http://blog.sina.com.cn/";		// 万一取不到uid，就去大博首。
			}
			*/
			if(msnLoginComeFrom){
		...
		/*
		function __getCurrectUID(){
			var arr = decodeURIComponent(Utils.Cookie.getCookie('SUP')).split('&');
			var keyValue, uid;
			for (var n = 0, m = arr.length; n < m; n++){
				keyValue = arr[n].split('=');
				if (keyValue[0] == 'uid'){
					uid = keyValue[1];
				}
			}
