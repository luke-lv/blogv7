...
_UTL.formatObjectData = function (obj) {
    ...
    for (var dat in obj) {
        switch (typeof(obj[dat])) {
            case "string":
                if (obj[dat] == "true")
                    obj[dat] = true; else if (obj[dat] == "false")
                    obj[dat] = false; else if (obj[dat].match(/^\d+$/))
...
_UTL.shadow.add = function (el, off, op) {
    ...
    if (typeof(osd.style.filter) != "undefined")
        osd.style.filter = "progid:DXImageTransform.Microsoft.Alpha(opacity=" + op + ")"; else
...
_UTL.gotRegInfo = function (omsg, module) {
    if (omsg.key == "settings") {
        ...
        if (module.oapp.settings && module.oapp.settings.comm && module.oapp.settings.comm[module.oifo.mname])
            module.oset = module.oapp.settings.comm[module.oifo.mname]; else
            module.oset = null;
...
_UTL.eventFollowShow = function (e, odiv, offx, offy) {
    ...
    if (iy + h > sy + tb.clientHeight) {
        iy -= h;
    }
    if (ix + w > sx + tb.clientWidth) {
        ix -= w;
    }
...
_UTL.setFixedStyle = function (el, btop, bleft) {
    if (_COMM.IE6 || ( navigator.appName.indexOf("Microsoft") != -1 && document.compatMode == "BackCompat" )) {
        var es = "top:expression(eval( window.document.compatMode && window.document.compatMode=='CSS1Compat') ? " + "window.document.documentElement.scrollTop " + (btop ? '' : '+ (window.document.documentElement.clientHeight - this.clientHeight)' ) + ' : ' + ' window.document.body.scrollTop ' + (btop ? '' : ' + (window.document.body.clientHeight - this.clientHeight) ' ) + ');';
...
_UTL.blind.show = function (z, clr, opt) {
    ...
    if (typeof(obd.style.filter) != "undefined")
        obd.style.filter = "progid:DXImageTransform.Microsoft.Alpha(opacity=" + opt + ")"; else
...
_UTL.setCookie = function (name, value, expires, path, domain, secure) {
    document.cookie = name + "=" + escape(value) + ((expires) ? "; expires=" + expires.toGMTString() : "") + ((path) ? "; path=" + path : "") + ((domain) ? "; domain=" + domain : "") + ((secure) ? "; secure" : "");
};
_UTL.getCookie = function (name) {
...
_UTL.deleteCookie = function (name, path, domain) {
    if (_UTL.getCookie(name)) {
        document.cookie = name + "=" + ((path) ? "; path=" + path : "") + ((domain) ? "; domain=" + domain : "") + "; expires=Thu, 01-Jan-70 00:00:01 GMT";
    }
};
_UTL.getUrlArg = function (akey, surl) {
    var re = new RegExp("[\\?|&]" + akey + "=([^&]*)(&|$)", "i");
    if (!surl) surl = document.URL;
    var m = surl.match(re);
    if (m && m[1]) {
        return unescape(m[1]);
    }
...
_ZXML.loadXMLStr = function (xmls) {
    ...
    if (window.ActiveXObject) {
        var arrSignatures = ["Microsoft.XmlDom", "MSXML2.DOMDocument.4.0", "MSXML2.DOMDocument.5.0",
...
_ZXML.init = function () {
    ...
    if (window.XMLHttpRequest) {
        try {
            _ZXML.req = new XMLHttpRequest();
        } catch (e) {
            _ZXML.req = false;
        }
    } else if (window.ActiveXObject) {
        try {
            _ZXML.req = new ActiveXObject('Msxml2.XMLHTTP');
        } catch (e) {
            try {
                _ZXML.req = new ActiveXObject('Microsoft.XMLHTTP');
            } catch (e) {
                _ZXML.req = false;
            }
        }
...
_COMM.init = function (commid) {
    ...
    if (!el) {
        ...
        var ts;
        var fv = (typeof(commid) == "string" ? "commid=" + commid + "&" : "") + 'setReadyState=_COMM.setReadyState&onLoad=_COMM.onLoadCallback&BC_JSCallback=_COMM.gotMessage&MP3_callBack=_MP3.onCallBack';
        if (_COMM.IE) {
            ts = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="_commFlash_ob" style="width:0px;height:0px;">' + '<param name="allowScriptAccess" value="always" />' + '<param name="allowNetworking" value="all" />' + '<param name="wmode" value="Window"/>' + '<param name="FlashVars" value="' + fv + '" />' + '<param name="movie" value="' + _COMM.staticServer + 'comm.swf' + (TT ? "?" + Math.random() : "") + '" /></object>';
            el.innerHTML = ts;
        } else {
...
_COMM.getAuthShortcut = function (module, oau) {
    ...
    if (oau && oau.bLogin > 0) {
        ...
        if (module.oifo.winInfo.hostPage.search(/sina\.com\.cn/i) == -1)
            module.auth.isMine = (module.owner.uid == oau.uid && oau.bLogin); else
...
_STATS.add = function (sid, desc, uid) {
    ...
    if (!uid) {
        if (_COMM && _COMM.auth) {
            if (_COMM.auth.uid)
                uid = _COMM.auth.uid; else if (_COMM.auth.sinaUserInfo && _COMM.auth.sinaUserInfo.uid)
...
var _ZDrag = {
    ...
    "start": function (e, el, dropfun, movefun) {
        ...
        if (_ZDrag.oblind.setCapture)
            _ZDrag.oblind.setCapture(); else if (window.captureEvents)
...
_WCHAT.getOfflineMessage = function (bclear) {
    var fn = _WCHAT.server.head + _WCHAT.server.offlinemsg + "?id=" + _WCHAT.myChatID + (typeof(bclear) != "undefined" && bclear == false ? "&clear=false" : "");
        ...
        case -2:
            ...
            if (m && m[1] && m[2] && typeof(m[3]) != "undefined") {
                ...
                if (m[1] == _WCHAT.myChatID)
                    _WCHAT.block.list[m[2]] = (lv > 0); else if (m[2] == _WCHAT.myChatID)
...
_WCHAT.getStatusName = function (stat, chatid) {
    ...
    if (stat >= 10 && stat < 20)
        ols = "away"; else if (stat >= 20 && stat < 30)
        ols = "busy"; else if (stat == -10)
        ols = "hidden";
    return ols;
};
...
_WCHAT.checkMsg = function () {
    ...
    if (_WCHAT.bPaused() || _WCHAT.currQID)
        return null;
    var fn = _WCHAT.server.head + _WCHAT.server.getmsg + '?id=' + _WCHAT.myChatID
//		+ '&p=' + _WCHAT.myInfo.p
//		+ '&uid=' + _WCHAT.myInfo.uid
        + "&a=" + _WCHAT.lastMsgID + "&var=__wchatMessages" + "&hid=" + _WCHAT.hid
...
_WCHAT.callSvr = function (jssrc, cbfun, pri) {
    ...
    if (ojs) {
        ...
        if (rqid)
            _WCHAT.cleanUPQID(rqid, false); else
            document.body.removeChild(ojs);
    }
    if (_WCHAT.currQID)
...
_WCHAT.siftMessage = function (msg) {
    if (_WCHAT.re.revMessage)
        return msg.replace(_WCHAT.re.revMessage, "(feid)"); else
        return msg;
};
...
_WCHAT.sendMessage = function (to, msgStr, cmsgID, type) {
    ...
    _COMM.sendXML(_WCHAT.server.sendmsg + "?id=" + to + '&c=' + cmsgID + (type ? '&t=' + type : "") + '&uid=' + _WCHAT.myInfo.uid + '&ut=' + _WCHAT.myInfo.p, xmls, _WCHAT.sendMessageCallback);
