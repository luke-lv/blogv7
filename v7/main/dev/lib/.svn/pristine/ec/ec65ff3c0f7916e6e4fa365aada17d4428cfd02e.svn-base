...
//验证码引入
(function() {
	//var checkManager_open_msg=null;
	if(scope.blogOpener) {
	...
	openBlog.prototype = {
		...
		getCon : function() {
			var checkcodeHTML = ['<div class="boxA" style="margin: 20px 0; overflow: hidden; zoom: 1;">'
				,'<div id="checkCodeBox_open_msg" class="check-code-box clearfix" > </div>'
				,'</div>'].join('');
			return [
		...
		,_initDialog : function() {
			...
			var checkCodeBox_open_msg={
				id:'#checkCodeBox_open_msg'
			}
			checkManager_open_msg=new Lib.checkManager(checkCodeBox_open_msg);
			this.bindEvent();
		...
		,bindEvent : function() {
			...
			Core.Events.addEvent($E('openBlogOkBtn_t'),function(){
				//验证码为空或者新版验证码无效
				if(!checkManager_open_msg.validate()) {
	...
	function whenOpenBlog(opt) {
		...
		var data_my=checkManager_open_msg.getPostData(my_data);
