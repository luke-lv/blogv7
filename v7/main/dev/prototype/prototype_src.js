...
var Hash = function (object) {
    if (object instanceof Hash) this.merge(object); else Object.extend(this, object || {});
};
...
Hash.toQueryString.addPair = function (key, value, prefix) {
    ...
    if (value === undefined) this.push(key); else this.push(key + '=' + (value == null ? '' : encodeURIComponent(value)));
}
...
Object.extend(Hash.prototype, {
    ...
    remove: function () {
        ...
        for (var i = 0, length = arguments.length; i < length; i++) {
            ...
            if (value !== undefined) {
                if (result === undefined) result = value; else {
...
Element.extend = function (element) {
    ...
    if (!element || !element.tagName || element.nodeType == 3 || element._extended || F.SpecificElementExtensions || element == window)
        return element;
...
Element.addMethods = function (methods) {
    ...
    if (!tagName) Object.extend(Element.Methods, methods || {}); else {
        if (tagName.constructor == Array) tagName.each(extend); else extend(tagName);
    }
...
Form.Methods = {
    ...
    findFirstElement: function (form) {
        return $(form).getElements().find(function (element) {
            return element.type != 'hidden' && !element.disabled && ['input', 'select',
...
Form.Element.Methods = {
    ...
    activate: function (element) {
        ...
        try {
            element.focus();
            if (element.select && (element.tagName.toLowerCase() != 'input' || !['button', 'reset',
                'submit'].include(element.type)))
                element.select();
        } catch (e) {
        }
        return element;
