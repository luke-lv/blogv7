...
$import("tempLib/insertSmiles/smilesTemplate.js");
...
App.smilesDialog2.prototype = {
    ...
    showTab: function (nPage, nCount) {
        ...
        for (var i = 0; i < nCount; i++) {	// 逐个创建左侧 Tab
            ...
            var tab = new Tab('<img src="http://www.sinaimg.cn/uc/myshow/blog/misc/gif/' + _sortId + '.gif" height="20" width="20" align="absmiddle"><div><span><a href="#" onclick="return false;" title="' + _name + '">' + _name + '</a></span><em>(' + _data.length + ')</em></div>', {
                    isFocus: false,
                    className: "cur"
                });
            tab.addOnAbort(Core.Function.bind3(function (tab) {
            }, null, [tab]));
...
//		trace(">>显示表情 第" + nPage + "页，每页" + nCount + "个");
        ...
        this.dialogNodes.magicTip.style.cssText = "display:none;";
        for (var i = 0; i < nCount; i++) {	//	逐个写入表情
            ...
            _a.onmousedown = Core.Function.bind3(function (nCode, sName, swfName) {
                __this.hide();
                __this.option.callback(nCode, sName, swfName);
            }, null, [_smile.code, _smile.name, _smile.swf || null]);
            _a.innerHTML = '<img style="width:50px;height:50px;" src="http://www.sinaimg.cn/uc/myshow/blog/misc/gif/' + _smile.code + 'T.gif" alt="' + _smile.name + '" title="' + _smile.name + '" />';
            _li.appendChild(_a);
            ...
            if (_smile.swf) {
                //alert(_smile.swf);
                var _div = $C("div");
                _div.innerHTML = '<a href="#"   onclick="window.$magicFacePlay(\'' + _smile.swf + '\');return false;"><img height="13" title="播放表情" align="absmiddle" width="14" alt="" src="http://simg.sinajs.cn/blog7style/images/common/img_arrow_r.gif"></a>';
                _div.className = 'arr_r';
                _li.appendChild(_div);

                //tips	提示魔法表情
                this.dialogNodes.magicTip.style.cssText = "display:block;";
            }
...
//                    + '<a href="#" onclick="return false;" title="向下" class="down_off"></a>';
    ...
    eventListen: function () {
        ...
        Core.Events.addEvent(document.body, function (cls) {
            ...
            if (__this.dialogNodes.entity != null && (Core.Dom.contains(__this.dialogNodes.entity, _eventTarget) == false || cls == true)) {
                __this.hide();
            }
        });
        // 浮层关闭事件
...
//				alert("请输入关键字");
    ...
    getKeyWord: function () {
        ...
        var __this = this;
        var smileData = Utils.Io.JsLoad.request("http://www.sinaimg.cn/uc/myshow/blog/misc/gif/smileKeywordConfig.js?varname=smileKeywordConfig", {
                onComplete: function (oData) {
                    ...
                    for (var i = 0, len = _hotkey.length; i < len; i++) {// 创建推荐的热门关键字链接
                        ...
                        if (_hotkey[i] == "魔法表情") {
                            //<img height="15" align="absmiddle" width="15" title="" src= c=>
                            var img = $C("img");
                            img.style.cssText = "height:15px; width:15px;";
                            img.setAttribute("align", "absmiddle");

                            img.src = "http://simg.sinajs.cn/blog7style/images/common/sg_trans.gif";
                            img.className = "SG_icon SG_icon68 img_m";
                            _hotDom.appendChild(img);
                        }
    ...
    searchKey: function (sHotkey) {
        ...
        for (var i = 0; i < _len; i++) {
            if (_resultArray.indexOf(_allData[i].code) != -1 || _allData[i].sname.indexOf(sHotkey) != -1 || _allData[i].name.indexOf(sHotkey) != -1) {
