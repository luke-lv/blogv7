...
Comment2TSina.prototype = {
	...
	//评论id
	data: null,
	initialize: function(anonyous, data){
		this.anonyous=anonyous || false;
		this.data=data||{};
	...
	writeCommTips: function(){
		if( !$E("bb") ){ return; } //没有这个说明PHP还没有上线，上线后去掉本行代码即可

		var html='';
		var tips=$E("writeComm_tips");
		
		if( !tips ){
			var tips_box = $C("div");
			tips_box.className = "writeComm_tips_box";
			tips_box.style.display = "none";
			tips_box.innerHTML='<span class="writeComm_tips" id="writeComm_tips"><strong></strong><cite><a href="javascript:;">关闭</a></cite></span>';
			var writeComm=Core.Dom.byClz(document, "div", "writeComm");
			if(writeComm){
				writeComm[0].insertBefore(tips_box, writeComm[0].childNodes[0]);
			}else{
		...
		trace($E("anonymity").checked);
		if( this.anonyous || $E("anonymity").checked || this.data.re_weibo==="no_blog"){ //tip2
			content.innerHTML = '评论发表成功，<a href="javascript:void(0);">点此还可以同步到微博，分享给我的粉丝。</a>';
			tbox.style.display="block";
		}else if(this.data.re_weibo==="success"){ //tip1
	...
	bindTipsEvents: function(tips){
		...
		Core.Events.addEvent(tips.firstChild, function(event){
			...
			if( txt==="点此查看"){
				...
				$E("writeComm_tips").parentNode.style.display="none";
				this.bindToTSina();
			}else{
...
&oauth_callback=http%3A%2F%2Fcontrol.blog.sina.com.cn%2Ft_sina_blog%2Fcallback.php
	...
	bindToTSina: function(){
		...
		var url='http://control.blog.sina.com.cn/blogprofile/tbind.php?from=js&domain='+document.domain;
		//console.log("url="+url);
	...
	shareToT: function(){
		...
		if(shareWord){
			...
			var title = encodeURIComponent(shareWord+"——评论" + nickName + '的博文：' + Core.Dom.byClz(document.body, 'h2', 'titName')[0].innerHTML)
			*/
			...
			if (imgCon) {
				...
				u += '&pic=' + encodeURIComponent(img.src);
			}
