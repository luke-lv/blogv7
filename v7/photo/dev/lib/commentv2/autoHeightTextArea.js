 ...
 *      'textArea': textarea,
 *      'maxHeight': 100,
 *      'inputListener': function(){trace('--autoHeightTextArea--');}
 * });
 */
$import("lib/commentv2/_comment.js");
$import("sina/core/dom/getStyle.js");
$import("sina/core/events/addEvent.js");
CommentV2.autoHeightTextArea = function(option) {
    ...
    function getTextAreaHeight(el) {
        ...
        if ($IE) {
            var paddingHeight = parseInt(getStyle(el, "paddingTop"), 10) + parseInt(getStyle(el, "paddingBottom"), 10);
            snapHeight = Math.max(el.scrollHeight - paddingHeight, el.defaultHeight);
        } //如果不是ie则需要先创建一个对像的副本
        else {
            var paddingHeight = $MOZ ? 0 : parseInt(getStyle(el, "paddingTop"), 10) + parseInt(getStyle(el, "paddingBottom"), 10);
            ...
            snapHeight = Math.max(textArea.scrollHeight - paddingHeight, el.defaultHeight);
    ...
    var textArea = option.textArea,
        ...
        inputListener = option.inputListener,
        ...
        (listener = function() {
            ...
            setTimeout(function() {
                var nSnapHeight = getTextAreaHeight(textArea) || 0,
                    viewHeight;
                ...
                textStyle.height = Math.min(maxHeight, nSnapHeight) + "px";
            }, 0);
        })()
        // if (!$IE) {
        //     try {
        ...
        //对TextArea进行事件绑定
        if (!textArea.binded) {
            addEvent(textArea, listener, 'keyup');
            addEvent(textArea, listener, 'focus');
            addEvent(textArea, listener, 'blur');
