...
Lib.Invite_Base.prototype = {
		"html"	: ['<div class="CP_layercon2 addFriendItem">'
			...
			,'<div class="formTowho">'
				...
				,'<div id="checkCodeBox" class="check-code-box clearfix" style="padding: 29px 0px 9px;"></div>'
				,'</div>'
				,'<div class="btn">'
	...
	,initialize	: function (sUid, onlineTime) {
		...
		var checkCodeBox_invite={
			id:'#checkCodeBox'
		}
		checkManager=new Lib.checkManager(checkCodeBox_invite);
		if (this.dialog == null) {
//			Debug.log("对话框尚不存在");
...
//		this.dialog.setFixed(true);
	...
	checkData	: function () {
		...
		// 验证码是否为空
		this.checkcode = checkManager.validate();
	...
	,"postContent"	: function () {
		...
		var param_m = {
			tuid		: this.uid
			,fuid		: $UID
			//,authcode	: Core.String.trim($E("invite_checkcode").value)旧的验证码需要的
