...
(function(proxy){
	...
	var ajaxCache = function(url,mothed){
		...
		ajax.request = function(args){
			...
			if(json){
				setTimeout(function(){args['onSuccess'](json);},1);
			}else{
	...
	(function(_p){
		...
		_p.press = function(e){
			...
			switch(ev.keyCode){
				case KEYENTER	: this.ok();break;
				case KEYESC		: this.clear();break;
				case KEYUP		: this.up();break;
				case KEYDOWN	: this.down();break;
				case KEYLEFT	: this.back();break;
				case KEYRIGHT	: this.ok();break;
		...
		_p.looping = function(){
			...
			if(this.keyText == _inputValue){
				...
				if(typeof this.data == 'string'){
					...
					this.trans.request({
						'GET' : postData,
						'onSuccess' : bind(function(json){this.show(json);},this),
						'onError' : bind(function(){this.box.style.display = 'none';},this)
					});
				}else{
		...
		_p.endLoop = function(){
			setTimeout(bind(function(){this.infoEle.style.display = 'none';},this),50);
			try{clearInterval(this.timer);}catch(exp){}
			setTimeout(bind(this.hidd,this),100);
		};
