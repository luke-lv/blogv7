...
cateDialog.prototype= {
	...
	initDialog:function(){
		...
		this.manage = null;
		this.manage = new cateMng(this.id, this.itemTpl);
	...
	tpl:[
		'<div id="categoryBody">',
			...
			'<div id="categoryHead">',
				'<table>',
				  	'<tr>',
						...
						'<td><span class="SG_txtc">请用中文、英文或数字。</span></td>',
				  	'</tr>',
				'</table>',
				'<div id="errTips"></div>',
			'</div>',
			'<form method="post" id="categoryForm" name="form">',
				'<div id="categoryList">',
				  	'<ul class="clearfix" id="datalist">&nbsp;</ul>',
	...
	itemTpl:[
		'<li class="{class}">',
			...
			'<span id="ctrl_{index}" class="control">',
				...
				'</div>',
			'</span>',
			'<div id="editctrl_{index}" class="writeinfo" style="display:none">',
...
cateMng.prototype = {
	 ...
	 */
	initialize:function(id, itemTpl){
		this.id = id;
