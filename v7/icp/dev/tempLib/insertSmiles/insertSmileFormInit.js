...
App.insertSmilesForm2.prototype = {
	...
	renderUI			: function () {
		...
		for(var key in this.data){
			...
			if(count <= this.option.sortCount){
				...
				node.onmousedown = Core.Function.bind3(function (key) {
							App.insertSmilesDialog2(key, __this.option.clickCallback, __this.option.positionNode,
		...
		_morelink.onmousedown = (function (ele) {
									return function() {
										App.insertSmilesDialog2(0, __this.option.clickCallback, __this.option.positionNode,
											__this.option.arrPosPix);
   ...
   // <div style="position:absolute; top:37px; left:36px;"><a href="#"><img src="../../images/common/img_arrow_r.gif" width="14" height="13"  alt="" align="absmiddle" /></a></div>
				var tempHTML='<a href="#"><img src="http://www.sinaimg.cn/uc/myshow/blog/misc/gif/' + _code + 'T.gif" alt="'
						+ _title + '" title="' + _title + '" height="50" width="50"/></a>';
				if(_swfName){
...
App.insertSmilesDialog3 = function (sTextarea, sSortNode, sRecommNode, fCallBack, sPositionNode, arrPix,sFrame,opt_evnets) {
	fCall = function(nCode, sName,swfname) {
		/*setTimeout(function(){
			scope.commEditor.insertHTML(sFrame,'<img src="http://www.sinaimg.cn/uc/myshow/blog/misc/gif/'
										+ nCode 
										+ 'T.gif" alt="'
										+ sName
										+ '" title="'
	...
	App.insertSmilesDialog2.callback = fCall;
	if(scope.smileDialog == null){
		scope.smileDialog = new App.smilesDialog2();
	}
	scope.smileDialog.setSort(0, fCall);
	var xy = Core.Dom.getXY($E(sPositionNode));
	var x = xy[0] + arrPix[0];
	var y = xy[1] + arrPix[1];
	scope.smileDialog.setPosition(x, y);
 ...
 */
App.formInsertSmile2 = function (sTextarea, sSortNode, sRecommNode, fCallBack, sPositionNode, arrPix,sFrame,opt_evnets) {
...
commEditor.prototype = {
	init : function(sFrame,cssStr) {
		var doc = this.docs[sFrame];
		try {
			doc.body.designMode = 'on';
		} catch(e){}
		// doc.open(); //这个方法在火狐中报错 zhihang1@20150731
		//this.doc.writeln('<html><head style></head><body style="width:100%;height:100%" designMode="on"></body></html>');
		// doc.writeln(this.getEditorHTML('',cssStr));
		// doc.close();
		doc.body.style.cssText = cssStr;
		doc.body.contentEditable = true;
	},
	
	lazyInit : function(fId,cssStr) {
		var me = this;
		return function() {
	...
	append : function(sFrame,sTextarea,actions) {
		...
		function getDoc() {
			try {
				...
				// if($IE) { //IE中造成bug－http://issue.internal.sina.com.cn/browse/BLOGWEB-140 zhihang1@20150814 
				// 	me.docs[sFrame].body.onclick = me.lazyInit(sFrame,actions.cssStr);
				// } else {
					(me.lazyInit(sFrame,actions.cssStr))();
				// }
			} catch(e) {
	...
	clearHTML : function(fId) {
		...
		doc.body.innerHTML = '';
		//以下两行为了去除：FF下评论里加图片，图片正在编辑时就提交，提交了编辑框还会留下
	...
	insertHTML: function(fId,s_html){
		...
		if ($IE) {
			...
			if(($CHROME || $SAFARI) 
				&& (encodeURIComponent(doc.body.innerHTML) == '%0A' || doc.body.innerHTML == '')) {
				doc.body.focus();
			}
	 ...
	 */
	getEditorHTML : function(sContent,cssString) {
		var html = "";
		html += "<html>";
		html += "<head>";
