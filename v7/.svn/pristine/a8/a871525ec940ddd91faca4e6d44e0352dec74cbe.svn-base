...
/**
 * @fileoverview 在博文页加入淘宝广告
 *
 * @create 2012-12-24
 * @author Qiangyee
 */
$registJob("showTB680", function (){
    // 找到第一列
    // var col1  = document.getElementById("column_1");

    // var comps = Core.Dom.byClass("SG_conn","div",col1);

    // var insertEl = comps[2] || comps[comps.length - 1];
    // 插入淘宝广告埋点
    ...
    // Core.Dom.insertHTML(insertEl, html, 'beforebegin');
    Lib.execZhitou(function(res) {   // 改为pdps码投放 zhihang1@2014-12-31
        ...
        var adEl = blogAd.insertIns('PDPS000000033239', insertEl, 'beforebegin');
        (window.sinaads || []).push({
            element: adEl,
            params: {
