...
$registComp(901, {
    ...
    "load": function() {
            ...
            if (scope.$pageid != "fakeIndex") { //此处添加判断条件，是排除封杀用户昵称不读取
    ...
    "showNickName": function(oResult) {
            ...
            var nickEl = $E("comp_901_nickname");
            nickEl.innerHTML = nickname;
            scope.owenerNickName = nickEl.textContent || nickEl.innerText;
    ...
    "loadOtherInfo": function() {
        ...
        //判断用户是否要更换新徽章--liming9 已经不再显示搬家徽章
    ...
    "loadPv": function() {
        ...
        if (scope.totalPv && !isNaN(scope.totalPv)) {
            ...
            if (scope.$articleid) {
                ...
                // pvurl = "http://hits.blog.sina.com.cn/hits?act=3&uid=" + $uidhex + '&ref='+refer;
    ...
    "loadBadge": function(data) {
        // debugger;
        var names = ["点亮荣誉勋章", "新新博主", "新锐博主", "先锋博主", "精英博主", "风云博主", "资深博主", "元老博主", 0, 0, "图片博主", "兑换图片博主服务", "私密博文",
            "兑换私密博文", "金笔", "兑换金笔"
        ];
    ...
    loadGoldpan: function() {
            ...
            if (dEl && rEl) {
                Utils.Io.JsLoad.request("http://comet.blog.sina.com.cn/api?maintype=goldpen&uid=" + scope.$uid, {
                    onComplete: function(data) {
    ...
    "loadScore": function() {
        ...
        /*
         * 载入博主关注数
         * 添加荣誉徽章载入，主要借用这里的接口，然后调用loadBadge方法 jiangwei5
         */
        ,
    ...
    "showVisitButton": function() {
        ...
        if ($E('info_locate_id')) {
            ...
            var result = ['<div id="info_locate_id" class="info_locate"><div class="SG_j_linedot"></div>', '<div class="info_btn2">', '<p><a href="#" onclick="' + inviteButtonHTML, '</cite></a>', '<a href="#" onclick="Lib.scrip(' + scope.$uid + ');return false;" class="SG_aBtn">', '<cite>发纸条</cite></a></p>',
                '<p><a href="http://blog.sina.com.cn/s/profile_' + scope.$uid + '.html#write" class="SG_aBtn" target="_blank">', '<cite>写留言</cite></a>',
                '<a id="module_901_attention" href="#" onclick="return false;" class="SG_aBtn' + (this.isAttentioned ? ' SG_aBtn_dis' : '') + '">',
                '<cite onclick="Lib.Component.' + (this.isAttentioned ? ' isAttentioned' : 'Attention') + '();">' + (this.isAttentioned ? '已' : '加') + '关注</cite></a></p>', '<div class="clearit"></div>', '</div></div>'
            ].join("");
