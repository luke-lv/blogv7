...
App.smilesDialog2.prototype = {
	...
	dataSettle	: function (oData, nCount) {
		...
		if(this.allDataBack == null&& this.allData.length > 0){	// 全部表情的备份，用于表情搜索
	...
	showTab		: function (nPage, nCount){
		...
		for(var i = 0; i < nCount; i ++){	// 逐个创建左侧 Tab
			...
			var tab = new Tab('<img src="http://www.sinaimg.cn/uc/myshow/blog/misc/gif/' + _sortId
						+ '.gif" height="20" width="20" align="absmiddle"><div><span><a href="#" onclick="return false;" title="' + _name + '">'
						+ _name + '</a></span><em>(' + _data.length + ')</em></div>'
				, {
	            	isFocus		: false,
	            	className	: "cur"
				});
...
//		trace(">>显示表情 第" + nPage + "页，每页" + nCount + "个");
		...
		for(var i = 0; i < nCount; i ++){	//	逐个写入表情
			...
			_a.onmousedown = Core.Function.bind3(function (nCode, sName,swfName){
				__this.hide();__this.option.callback(nCode, sName,swfName);
...
//				alert("请输入关键字");
	...
	getKeyWord	: function () {
		...
		var __this = this;
		var smileData = Utils.Io.JsLoad.request(
			"http://www.sinaimg.cn/uc/myshow/blog/misc/gif/smileKeywordConfig.js?varname=smileKeywordConfig",
		{
			onComplete	: function(oData){
