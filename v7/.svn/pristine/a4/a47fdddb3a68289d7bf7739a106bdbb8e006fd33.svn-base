...
UploadLoading.prototype = {
	...
	setError:function(error,tip){
		...
		var ele=$E(this.ERROR_ID+this.id),
            imgItemEl = $E(ImageList.prototype.ITEM_ID+this.id)
		if(error){
			ele.innerHTML=tip||$SYSMSG["A77773"];
			ele.style.display="";
			this.state=3;
            // 图片错误也认为是成功
            imgItemEl.setAttribute("isadd", "yes");
		}else{
			ele.innerHTML="";
			ele.style.display="";
		}
...
//		$E(this.DEL_ID+this.id).style.display="";
	...
	cancel:function(){
        //console.log("[UploadLoading -> cancel] "+this.ELE_ID+this.id);
	...
	initElement:function(){
		var loadingTemplate='<p id="#{TIP_ID}#{id}">0%</p>\
         <div class="loadbanner">\
         	<div  style="width:0%;" id="#{RATD_ID}#{id}"class="loadbanner_c"></div>\
         </div>';
        this.setError(false);
        var loadEl = $E(this.TIP_ID+this.id);
        if(!loadEl){
            var l_t=new Ui.Template(loadingTemplate);
            var html = l_t.evaluate({
                            id:this.id, 
                            RATD_ID:this.RATD_ID, 
                            TIP_ID:this.TIP_ID
                        });
            Core.Dom.addHTML($E(this.ELE_ID+this.id), html);
        }
	}
};
