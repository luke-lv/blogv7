...
$registJob("importV", function (){
    ...
    var _this = {
        ...
        getWeiboInfo : function(){
            ...
            Utils.Io.JsLoad.request(url, {
                onComplete  : function(result){
                    ...
                    if ("A00003" === result.code) {
                        var openBtn = $E("openBlogBtnTray");
                        if (openBtn) {
                            if ($IE){
                                openBtn.click();
                            } else {
        ...
        syncNickName:function(nick){
            Utils.Io.JsLoad.request("http://control.blog.sina.com.cn/blog_nickunify/qianzhanNick.php", {//此处调用同步昵称接口
                GET: {
                    uid:$UID,
                    nick:nick
                },
        ...
        comfirmImport : function(blogerInfo){
            ...
            winDialog.confirm('微博身份导入后，博客昵称将同步为微博昵称。',{
                ...
                funcOk : function(){
                    _this.syncVInfo();
                    _this.syncNickName(blogerInfo.nick);
                },
                funcCancel : function(){
    ...
    scope.___importV = function(){
        location.href = location.href+"#!import-v";
    ...
    // 开通博客后刷新本页弹出加V确认浮层
    if ("blogv" === scope.$pageid && "#!import-v" === location.hash) {
        _this.onImportV();
    }
});
