...
scope.Experience={
	...
	initialize:function(){
		...
		function appendSchool(){
			...
			td2.innerHTML='<div class="posSchool"><span class="tdP20">'+
											'<em>'+
												'<select disabled="disabled" id="schoolType'+schoolIndex+'" style="width:66px;">'+
													'<option value="0">大学</option>'+
												'</select>'+
											'</em>'+
											'<em>'+
												'<input style="color:#999999;" id="schoolName'+schoolIndex+'" type="text" class="personText" value="学校名称"/>'+
												'<div class="assn_sel" style="left:11110px; top:35px;"></div>'+
											'</em>'+
											'<em>'+
												'<input style="color:#999999;" type="text" id="profession'+schoolIndex+'" class="personText" value="学院/专业"/>'+
											'</em>'+
											'<em>'+
												'<select id="matrDate'+schoolIndex+'" style="width:86px;">'+
													'<option value="0">入读时间</option>'+
												'</select>'+
											'</em>'+
											'<em class="sameBirth">'+
												'<a style="display:none;" id="findSchoolmate'+schoolIndex+'" href="javascript:;">找同学&gt;&gt;</a>'+
											'</em>'+
											'<em class="personDelete">'+
												'<a class="CP_a_fuc" href="javascript:;">[<cite id="btnRemoveSchool'+schoolIndex+'">删除</cite>]</a>'+
											'</em></span>'+
											'<p style="display:none;" id="msgFindSchoolmate'+schoolIndex+'" class="errorAction addForm">请输入正确的学校名称，才能找到你的同学</p>'+
											'<p style="display:none;" id="msgSchoolName'+schoolIndex+'" class="errorAction addForm">请输入学校名称</p>'+
											'<p style="display:none;" id="msgMatrDate'+schoolIndex+'" class="errorAction addForm">请选择入读时间</p>'+
											'<p style="display:none;" id="msgAddNewSchool'+schoolIndex+'" class="errorAction addForm">未找到该学校，请输入正确的学校名称。</p>'+
										'</div>';
			...
			//初始化"入学时间"下拉列表
			selectAppenderDesc.initialize($E("matrDate"+schoolIndex), new Date().getFullYear(), 1950);
...
//				,'http://icp.cws.api.sina.com.cn/profile/relate_school.php',{
				...
				'lightOnFun' :null,
				'lightOffFun' : null,
				'searchNullText' : '未找到该学校，请检查输入是否正确。',
...
//				,'http://icp.cws.api.sina.com.cn/profile/relate_college.php',{
		...
		function appendCompany(){
			...
			td2.innerHTML='<span class="tdP20">'+
							'<em>'+
								'<input id="companyName'+companyIndex+'" type="text" style="width: 171px;color:#999999" value="公司名称" class="personText"/>'+
							'</em>'+
							'<em>'+
								'<select id="fromYear'+companyIndex+'" style="width: 66px;"><option value="0">年</option></select>'+
							'</em>'+
							'<em>'+
								'<select id="fromMonth'+companyIndex+'" style="width: 46px;"><option value="0">月</option></select>'+
							'</em> 至 '+ 
							'<em>'+
								'<select id="toYear'+companyIndex+'" style="width: 66px;"><option value="today">今</option></select>'+
							'</em>'+
							'<em>'+
								'<select id="toMonth'+companyIndex+'" style="width: 46px;"><option value="0">月</option></select>'+
							'</em>'+
							'<em class="sameBirth">'+
								'<a style="display:none;" id="findWorkmate'+companyIndex+'" href="javascript:;">找同事>></a>'+
							'</em>'+
							'<em>'+
								'<a id="btnRemoveCompany'+companyIndex+'" href="javascript:;" class="CP_a_fuc">[<cite>删除</cite>]</a>'+
							'</em>'+
							'</span>'+
							'<p style="display:none;" id="msgInputCompanyName'+companyIndex+'" class="errorAction addForm">请输入公司名称</p>'+
							'<p style="display:none;" id="msgFindWorkmate'+companyIndex+'" class="errorAction addForm">请输入正确的公司名称，才能找到你的同事</p>'+
							'<p style="display:none;" id="msgYearMonth'+companyIndex+'" class="errorAction addForm">请选择入职完整年月份</p>'+
							'<p style="display:none;" id="msgLeaveYearMonth'+companyIndex+'" class="errorAction addForm">请选择离职完整年月份</p>'+
							'<p style="display:none;" id="msgErrorDate'+companyIndex+'" class="errorAction addForm">离职时间不应早于入职时间</p>';
			...
			//初始化年份下拉列表
			selectAppenderDesc.initialize($E("fromYear"+companyIndex), new Date().getFullYear(), 1908);
			selectAppender.initialize($E("fromMonth"+companyIndex), 1, 12);
			selectAppenderDesc.initialize($E("toYear"+companyIndex), new Date().getFullYear(), 1908);
...
//			(new Interface("http://icp.cws.api.sina.com.cn/profile/addSchool.php", "jsload")).request({
			(new Interface("http://control.blog.sina.com.cn/riaapi/profile/addSchool.php", "jsload")).request({
					...
					onSuccess : function (iData) {
						winDialog.alert("已成功提交申请，请等候审批",{ icon : "03" });
					},
		...
		function saveData(){
			...
			for(i=0;i<len;i++){
				...
				//入职年月份
				if($E("companyEditor").rows[i].getElementsByTagName("select")[0].value=="0"
				|| $E("companyEditor").rows[i].getElementsByTagName("select")[1].value=="0"){
				...
				&& $E("companyEditor").rows[i].getElementsByTagName("select")[3].value=="0"){
			...
			function _updateShowData(iData){
				...
				if(sds.length>0){
					...
					for(i=0;i<len;i++){
						...
						//有链接的情况
						//td2.innerHTML=' <span class="tdP20"><a id="sclLink_'+i+'" target="_blank" href="">'+sds[i]["schoolName"]+'</a></span>'+
						//				'<span class="tdP20"><a id="proLink_'+i+'" target="_blank" href="">'+sds[i]["profession"]+'</a></span>'+
						//				'<span class="tdP20">'+sds[i]["matrDate"]+'年入读</span>';
						...
						td2.innerHTML=' <span class="tdP20">'+sds[i]["schoolName"]+'</span>'+
										'<span class="tdP20">'+sds[i]["profession"]+'</span>'+
										'<span class="tdP20 SG_txtc">'+sds[i]["matrDate"]+'年入读</span>';
					}
				...
				if(cds.length>0){
					...
					for(i=0;i<len;i++){
						...
						//有链接的情况
						//td1.innerHTML='<span class="tdP20"><a target="_blank" href="http://uni.sina.com.cn/c.php?t=ren&st=1&gend=0&prov=0&city=0&age_min=0&age_max=0&ie=utf-8&comp='+encodeURIComponent(cds[i]["companyName"])+'">'+cds[i]["companyName"]+'</a></span>'+
						//				'<span class="tdP20">'+cds[i]["year1"]+'年'+cds[i]["month1"]+'月至'+newDate+'</span>';
										
						td1.innerHTML='<span class="tdP20">'+cds[i]["companyName"]+'</span>'+
										'<span class="tdP20 SG_txtc">'+cds[i]["year1"]+'年'+cds[i]["month1"]+'月至'+newDate+'</span>';
					}
...
//			(new Interface("http://icp.cws.api.sina.com.cn/profile/edit_career.php", "jsload")).request({
			(new Interface("http://control.blog.sina.com.cn/riaapi/profile/edit_career.php", "ajax")).request({
					POST : {
						uid:scope.$uid,
						school:schoolData,
						company:companyData
						//,nosearch: $E("experienceSearchMask").checked?"1":"0"
						,version:7
					},
					noencode:true,
					onSuccess : function (iData) {
		...
		function initializeInfo(iData){
			...
			for(i=0;i<len;i++){
				...
				if($E("companyEditor").rows[i].getElementsByTagName("select")[2].value=="today"){
					...
					$E("companyEditor").rows[i].getElementsByTagName("select")[3].style.display="";
				}
			}
