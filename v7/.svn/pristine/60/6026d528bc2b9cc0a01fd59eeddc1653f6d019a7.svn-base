...
Comment.Reply.prototype = {
    ...
    reply: function(id){
		...
		var iconBrother = replyBody.parentNode.parentNode;
		var spanWrap = SinaEx.firstChild(iconBrother);
		var pWrap = SinaEx.firstChild(spanWrap);
		var spans = SinaEx.next(pWrap);
		
		var iconWrapper = Core.Dom.getElementsByClass(iconBrother,"p","myReFrom");
		
		if(iconWrapper){
			var iconA = SinaEx.firstChild(iconWrapper[0]);
			var iconImg = SinaEx.firstChild(iconA);
			var prevForm = SinaEx.firstChild(iconBrother);
			var prevWrap = SinaEx.next(prevForm);
			var contents = prevWrap.innerHTML;
		}
		...
		function reply_btn_click(){
			_this.post(iconImg,contents,spans);
		}
     ...
     */
    post: function(icon,contents,spans){
		...
		var userid = spans.getAttribute("userid");
		if(icon && icon.className === "SG_icon SG_icon205"){
			data.from_pro = "q";
			if(contents){
				data.from_commentContent = contents;
			}
			if(userid){
				data.from_usr = userid;
			}
		}
