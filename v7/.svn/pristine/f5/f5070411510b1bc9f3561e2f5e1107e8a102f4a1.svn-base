...
PPTBox.prototype = {
    ...
    renderHTML : function(parame,width,height,idx) {
        ...
        var tpl_xg3 = '<p class="Ltit">'
                    +   '<a {{a_c_suda}} href="{{a_href}}" class="BNE_lkA" target="_blank" title="{{a_title}}"><i class="icon i7_dot"></i>{{words}}</a>'
                    + '</p>'
                    + '<p class="Lname">'
                    +   '<a {{a_c_suda}} href="{{a_href}}" class="BNE_lkB" target="_blank">{{author}}</a>'
                    + '</p>';
        ...
        if(this.materialtype === 1) {
            ...
            }else if(scope.$pageid == 'articleM_new' || scope.$pageid == 'article_new'){
                html = SinaBlog680.utils.tplToData(tpl_xg3, parame);
            }else {
