...
SinaEditor.PluginManager.register('imageBtn', function (options, currentEditor) {
    ...
    function createUploadBtn() {
        try {
            ...
            var hoverBtnHtml = ['<div id="#{panel}" style="', wrapperCSSText , '">'
                , '<i id="se-image-upload-btn" class="icon i21_img" style="_positon:absolute;_left:0px;_top:0px;"></i>'
                , '<div id="uploadimgflash" style="position:absolute;top:0px;left:0px;z-index:30;"></div>'
//        , '<input style="', inputCSSText, '" name="pic1" type="file" multiple accept="image/*" id="J_line_input">'
                , '</div>'].join('');

            var hoverBtn = new Layer({html: hoverBtnHtml});
...
//      });
    ...
    function createImg(url, filename) {
        ...
        var html = ['<div se-img="img-parent" ', extraAttr, '>', '<a se-img="link" ', extraAttr,
            ...
            '<span se-img="span" contenteditable="true">', IMG_TIPS, '</span>', '</div>'].join('');
    ...
    // flash上传结束回调
    function uploadFinish(fileName, result) {
        if (result.code === "A00006") {
            domUtil.each(result.data.pics, function (p, pic) {
                uploadImages.push({name: pic.name, pid: pic.pid});
                renderImage(pic);
            });
        }
    }
...
//
        ...
        var input = $dom.$('input[name="pic1"]', layer.getNode('panel'))[0];
        var insertParagraph;
        if (!input) {
            return;
        }
        function onInputChange(e) {
            var target = e.target, files = target.files, frag = domUtil.createFragment();
    ...
    function init() {
        ...
        var offsetLeft, imgToolbar
            , toolbarWith, toolbarHeight, isInputting;
...
//      }
            ...
            if (!uploadBtn) {
                ...
                offsetLeft = uploadBtn.node.offsetWidth + 10;
                bindClick(uploadBtn);
...
//            console.log('delteBtn');
        ...
        domUtil.on(currentEditor.doc.body, 'click', function (e) {
            e = domUtil.fixEvent(e);
            var target = e.target, seImg = domUtil.attr(target, 'se-img'), rng, start;
...
//                });
        ...
        currentEditor.on('beforeenter', function (e, evt) {
            var target = domUtil.fixEvent(evt).target, wrapper, newPrag, rng = currentEditor.getRange(), start = domUtil.getStart(rng), returnValue = true;
            ...
            if (domUtil.attr(target, 'se-img')) {
                ...
                if (returnValue) {
                    ...
                    var startContainer = rng.startContainer, resetStart;
        ...
        currentEditor.on('beforedelete', function (e, evt) {
            var target, offset = -1, wrapper, walker, resetStart, end, next, prev;
            var rng = currentEditor.getRange();
        ...
        currentEditor.on('afterCommand', function (e, cmdName) {
            if ('todo' === cmdName) {
                ...
                var images = domUtil.$('img'), imageName, pic;
...
//                deleteBtn = null;
    ...
    currentEditor.on('tmpAfterSetContent', function () {
        ...
        domUtil.each(imgs, function (i, img) {
            try {
                ...
                if (link && ('A' == link.nodeName.toUpperCase())
                    && imgParent != currentEditor.body
                    && domUtil.contains(currentEditor.body, imgParent)) {
    ...
    currentEditor.on('toolbar.beforeshow', function (e, toolbar, rng) {
        ...
        var start = domUtil.getStart(rng), end = domUtil.getEnd(rng);
