...
// with correct but slower alternatives.
	...
	function startingTag(element) {
		...
		try {
			return element[0].nodeType === TEXT_NODE ? lowercase(elemHtml) : elemHtml.match(/^(<[^>]+>)/)[1].replace(/^<([\w\-]+)/, function (match,
			                                                                                                                                  nodeName) {
...
/////////////////////////////////////////////////
	...
	function angularInit(element, bootstrap) {
		var elements = [element], appElement, module, names = ['ng:app', 'ng-app', 'x-ng-app',
			'data-ng-app'], NG_APP_CLASS_REGEXP = /\sng[:\-]app(:\s*([\w\d_]+);?)?\s/;
...
//////////////////////////////////
	...
	var jqCache = JQLite.cache = {}, jqName = JQLite.expando = 'ng' + new Date().getTime(), jqId = 1, addEventListenerFn = (window.document.addEventListener ? function (element,
	                                                                                                                                                                     type,
	                                                                                                                                                                     fn) {
...
//////////////////////////////////////////
	...
	forEach({
		...
		scope: function (element) {
			...
			return jqLite(element).data('$scope') || jqLiteInheritedData(element.parentNode || element, ['$isolateScope',
				'$scope']);
		},
			...
			forEach(type.split(' '), function (type) {
				...
				if (!eventFns) {
					if (type == 'mouseenter' || type == 'mouseleave') {
						var contains = document.body.contains || document.body.compareDocumentPosition ? function (a,
						                                                                                           b) {
							// jshint bitwise: false
 ...
 *     });
	...
	function $CompileProvider($provide, $$sanitizeUriProvider) {
		...
		this.$get = [
			'$injector', '$interpolate', '$exceptionHandler', '$http', '$templateCache', '$parse', '$controller',
			...
			function ($injector, $interpolate, $exceptionHandler, $http, $templateCache, $parse, $controller,
				...
				function compileTemplateUrl(directives, $compileNode, tAttrs, $rootElement, childTranscludeFn,
						...
						compile: function () {
							return {
								pre: function attrInterpolatePreLinkFn(scope, element, attr) {
									...
									(attr.$$observers && attr.$$observers[name].$$scope || scope).$watch(interpolateFn, function interpolateFnWatchAction(newValue,
									                                                                                                                      oldValue) {
	...
	function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument) {
		...
		return function (method, url, post, callback, headers, timeout, withCredentials, responseType) {
			...
			if (lowercase(method) == 'jsonp') {
				...
				var jsonpDone = jsonpReq(url.replace('JSON_CALLBACK', 'angular.callbacks.' + callbackId), callbackId, function (status,
				                                                                                                                text) {
...
// a pattern that is repeated a lot in the input validation logic.
	...
	function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
		...
		var listener = function (ev) {
			...
			if (ctrl.$viewValue !== value || // If the value is still empty/falsy, and there is no `required` error, run validators
			...
			link: function (scope, element, attr, ctrls) {
				...
				function setupAsOptions(scope, selectElement, ctrl) {
					...
					function render() {
						...
						var optionGroups = {'': []}, optionGroupNames = [''], optionGroupName, optionGroup, option, existingParent, existingOptions, existingOption, modelValue = ctrl.$modelValue, values = valuesFn(scope) || [], keys = keyName ? sortedKeys(values) : values, key, groupLength, length, groupIndex, index, locals = {}, selected, selectedSet = false, // nothing is selected yet
